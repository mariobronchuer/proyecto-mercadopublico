    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <nav className="mb-6">
        <ol className="flex items-center gap-2 text-sm text-slate-500">
          ...
          <li>/</li>
          <li><Link href="/licitaciones" className="hover:text-blue-600">Licitaciones</Link></li>
          <li>/</li>
          <li className="text-slate-900 font-medium">{codigo}</li>
        </ol>
      </nav>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 mb-6">
        <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
          <div className="flex items-center gap-3 flex-wrap">
            <span className="font-mono bg-blue-50 text-blue-600 px-3 py-1 rounded-lg text-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {licitacion.Comprador && (
            <div className="bg-slate-50 rounded-xl p-4">
              ...
              <p className="font-medium text-slate-900">{licitacion.Comprador.NombreOrganismo}</p>
              {licitacion.Comprador.NombreUnidad && (
                <p className="text-sm text-slate-600 mt-1">{licitacion.Comprador.NombreUnidad}</p>
              )}
              ...
              {licitacion.Comprador.DireccionUnidad && (
                <p className="text-sm text-slate-500 mt-1">{licitacion.Comprador.DireccionUnidad}</p>
              )}
            </div>
          )}
            ...
            <div className="space-y-2 text-sm">
              {licitacion.Fechas?.FechaPublicacion && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-slate-900">{new Date(licitacion.Fechas.FechaPublicacion).toLocaleDateString('es-CL')}</span>
                </div>
              )}
              {fechaCierreDate && (
                <div className="flex justify-between">
              ...
              {licitacion.Fechas?.FechaAdjudicacion && (
                <div className="flex justify-between">
                  <span className="text-slate-500">Adjudicación estimada:</span>
                  <span className="font-medium text-slate-900">{new Date(licitacion.Fechas.FechaAdjudicacion).toLocaleDateString('es-CL')}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
          ...
          <div className="space-y-4">
            {licitacion.Items.Listado.map((item, index) => (
              <div key={index} className="border border-slate-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-sm transition-all">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <h3 className="font-medium text-slate-900 mb-1">{item.NombreProducto}</h3>
                    <p className="text-sm text-slate-600">{item.Descripcion}</p>
                    <p className="text-xs text-slate-400 mt-2">{item.Categoria}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-bold text-blue-600">{item.Cantidad}</p>
                    <p className="text-sm text-slate-500">{item.UnidadMedida}</p>
                  </div>
                </div>
              </div>
            ))}
